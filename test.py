...
x = numpy.zeros([N+1, len(states)]) #pour stocker la clé du pricing optimal nécessaire pour obtenir cette meilleur espérance 
        ...
        for d in pricing_options :  #decision du nouveau pricing pour chacun des 2 vols [30,30] [30,50] [50,50] ....
            ...
            for r in range(2) : #le client choisi son vol en maximisant son utilité ou pas de vol, 0 = pas de vol, 1=v1 et 2=v2
                if r == 0: #s'il ne choisi pas de vol on reste dans le même état
                    state = i 
                    esp += 0.4*0 + f[t+1,states.index(state)] #le bénéfice associé à ce cas de figure est nul
                    ...
                    esp += 0.3*d[r-1] + f[t+1,states.index(state)] #le bénéfice associé à ce cas de figure est d[r-1]
